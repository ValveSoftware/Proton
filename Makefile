...
redist: configure
	...
	vagrant ssh -c 'make -C $(BUILD_DIR)/ redist && cp $(BUILD_DIR)/redist/* /vagrant/$(DEPLOY_DIR)'
	echo "Proton build available at vagrant_share/$(DEPLOY_DIR)"

deploy: configure
	mkdir -p vagrant_share/$(DEPLOY_DIR)-deploy
	vagrant ssh -c 'make -C $(BUILD_DIR)/ deploy && cp $(BUILD_DIR)/deploy/* /vagrant/$(DEPLOY_DIR)-deploy'
